...
let
  mkFlakeForTarget = { target, extraRustcFlags ? null, extraCargoArgs ? null }:
    let
  ...
  defaultFlake = mkFlakeForTarget { target = null; };

  extraTargetFlakes =
    let inherit (builtins) map listToAttrs;
    ...
    listToAttrs (map
      (t:
        let
          k = t.target;
          v = mkFlakeForTarget t;
        in
        nameValuePair k v)
      extraTargets);

in
defaultFlake // { extraTargets = extraTargetFlakes; }

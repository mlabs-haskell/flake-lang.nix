    ...
    rec {
      mkPackageSpec =
        ...
        let
          ...
          cabalPath =
            if length cabalFiles == 1 then
              src + "/${builtins.head cabalFiles}"
            else
              builtins.abort "Could not find unique file with .cabal suffix in source: ${src}";
          cabalFile = builtins.readFile cabalPath;
          parse =
            field:
            let
              lines = filter (s: if builtins.match "^${field} *:.*$" (toLower s) != null then true else false) (
                splitString "\n" cabalFile
              );
              line =
                if lines != [ ] then
                  ...
                  builtins.abort "Could not find line with prefix ''${field}:' in ${cabalPath}";
            in
      ...
      mkHackageNixFor =
        ...
        pkgs.runCommand "hackage-nix" { } ''
          ...
          }/bin/hackage-to-nix $out 01-index.tar "https://mkHackageNix/"
        '';
        ...
        { lib, ... }:
        {
          ...
          packages = pkgs.lib.listToAttrs (
            map (spec: {
              name = spec.pname;
              value = {
                src = lib.mkOverride 99 (copySrc spec.src);
              };
            }) pkg-specs
          );
        };
...
let
  ...
  ifd-parallel = pkgs.runCommandNoCC "ifd-parallel" {
    myInputs = builtins.foldl' (
      b: a:
      b
      ++ [
        a.extra-hackage
        a.extra-hackage-tarball
      ]
    ) [ ] theHackages;
  } "echo $myInputs > $out";

...
let
  ...
  cleanSrc =
    let
      filter =
        ...
        ) (craneLib.filterCargoSources path type) extraSourceFilters;
  ...
  vendoredSrc = pkgs.stdenv.mkDerivation {
    src = cleanSrc;
...
in
{
  ...
  packages =
    (optionalAttrs generateDocs {
      "${crateName}-rust-doc" = craneLib.cargoDoc (
        commonArgs
        // {
    ...
    // (optionalAttrs exportTests {
      "${crateName}-rust-test" = craneLib.buildPackage (
        commonArgs
        // {
          inherit cargoArtifacts;
          doCheck = false;
    ...
    // {
      "${crateName}-rust" = craneLib.buildPackage (
        commonArgs
        // {
          inherit cargoArtifacts;
          doCheck = false;
  ...
  checks =
    (optionalAttrs runTests {
      "${crateName}-rust-test" = craneLib.cargoNextest (
        commonArgs
        // {
    ...
    // (optionalAttrs runClippy {
      ...
      "${crateName}-rust-clippy" = craneLib.cargoClippy (
        commonArgs
        // {
          inherit cargoArtifacts;
        }
